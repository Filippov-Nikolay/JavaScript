...
if (confirm("2 + 2 * 2 = 6. Это верно?"))
    count += points[0];

if(confirm("Вы уверены?"))
    count += points[1];

if(confirm("Hello - это 'Привет'?"))
    count += points[2];
...
document.write(`За четвёртый вопрос - ${points[count++]} баллов. Верный ответ: <b>да</b> <br>`);
...
for(let i = 0; i < FIO.length; i++) {
    if(!(FIO[i] >= 'a' && FIO[i] <= 'z' || FIO[i] >= 'A' && FIO[i] <= 'Z') && FIO[i] != ' ' && FIO[i] != '.') {
        valid = false;
        break;
...
if(!valid)
    ...
    alert("Ввод - корректный");
document.write("<br><br>");


// ================ 3 ================
let urlAddress = "https://filippov-nikolay.github.io/MotoCompany/index.html?v=-NECnTb07dE&list=RDAMVM-NECnTb07dE";
let protocol = "";
let host = "";
let path = "";
let nameFile = "";
